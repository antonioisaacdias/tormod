...
export function Composer({
  ...
  const canSend = value.trim().length > 0 && !disabled && !working
    ...
    <div className="flex items-end gap-2.5">
      ...
      {working ? (
        <Button
          aria-label="Parar"
          onClick={onStop}
          className="size-12 shrink-0 rounded-2xl bg-danger text-white hover:brightness-110"
        >
          <X className="size-5" strokeWidth={2.5} />
        </Button>
      ) : (
        <Button size="icon" aria-label="Enviar" onClick={submit} disabled={!canSend} className="size-12 shrink-0 rounded-2xl">
