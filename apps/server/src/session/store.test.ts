...
describe("SessionStore", () => {
  it("persists, lists, and updates sessions", () => {
    ...
    store.upsert({ id: "s1", title: "uma", cwd: "/home/odin", status: "live", createdAt: "2026-06-10T00:00:00Z" });
    store.upsert({ id: "s2", title: "duas", status: "live", createdAt: "2026-06-10T00:01:00Z" });
  ...
  it("upsert updates an existing row without losing the Claude id", () => {
    const store = SessionStore.open(":memory:");
    store.upsert({ id: "s1", title: "v1", status: "live", createdAt: "t0" });
    store.setClaudeId("s1", "c1");
    store.upsert({ id: "s1", title: "v2", status: "closed", createdAt: "t0" });
